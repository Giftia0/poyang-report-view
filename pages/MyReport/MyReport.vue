...
<script setup>
	...
	import { getMyReport } from '../../http/api/report.js'
	...
	onLoad((opt) => {
		...
		getMyReport(category.value).then(res => {
	...
	const getReportByStatusAndTime = () => {
		getMyReport(category.value, dropdownStatus.value, dropdownTime.value, lastVisit).then(res => {
